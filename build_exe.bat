...
echo.
echo === MemOpt Launcher Desk : build EXE ===
echo.
...
taskkill /F /IM MemOptLauncherDesk.exe >nul 2>&1
timeout /t 2 /nobreak >nul

if exist "dist\MemOptLauncherDesk.exe" (
  del /f /q "dist\MemOptLauncherDesk.exe" >nul 2>&1
)
if exist "dist\MemOptLauncherDesk.exe" (
  echo [ERROR] Cannot replace dist\MemOptLauncherDesk.exe
...
echo.
echo OK
echo EXE: "%cd%\dist\MemOptLauncherDesk.exe"
echo.
if exist "dist\MemOptLauncherDesk.exe" explorer dist
pause
